...
const getApi = async () => {
  try {
    const res = await await axios.get('데이터를 받아올 주소의 뒷부분').then((res) => {
    });
  } catch (err) {
    console.error(err);
  }
...
무슨말이냐 하면, 클라이언트가 데이터를 어디다 요청하면 프록시가 이걸 보고 여기에 필요한 거구나 하고 넣어준다. 

'https://데이터를 받아올 주소/api/hamster' 가 있으면 axios에서는 '/api/hamster' 만 넣어서 요청하면 된다는 것이다. 

그러면 개발환경에서 깔끔하게 불러와진다.

## vercel proxy 설정

하지만 … axios에서 저런 식으로 작성하고 땡친다면, 배포환경에서는 그냥 'https://배포환경주소/api/hamster' 로 불러오게 된다. 
